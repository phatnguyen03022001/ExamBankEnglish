...
exports.getClasses = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy danh sách lớp học và giáo viên." });
  }
};
...
exports.getSemesters = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi lấy danh sách học kỳ cho giáo viên." });
  }
};
...
exports.getexamsubmissionsbyteacherid = async (req, res, next) => {
  try {
    ...
    if (!semesterId || !teacherId) {
      return res
        .status(400)
        .json({ message: "Semester ID and Teacher ID are required." });
    }
    ...
    if (classes.length === 0) {
      return res.status(404).json({
        message:
          "No classes found for the given teacher in the specified semester.",
      });
    }
...
exports.putexamsubmit = async (req, res) => {
  ...
  try {
    ...
    if (
      examSubmission.semesterID.toString() !== semesterId ||
      examSubmission.grade !== grade
    ) {
      return res
        .status(400)
        .json({ message: "ExamSubmission does not match the criteria" });
    }
...
exports.saveScores = async (req, res) => {
  ...
  try {
    ...
    const scoreUpdates = students.map(async (student) => {
      ...
      score.scores.fifteenMinutes = (student.fifteenMinuteScores || []).slice(
        0,
        3
      );
      ...
      const calculateAverageScore = () => {
        ...
        const fifteenMinuteScores = fifteenMinutes.filter(
          (score) => score !== null
        );
    ...
    res
      .status(500)
      .json({ message: "Failed to save scores", error: error.message });
  }
};
  ...
  try {
    ...
    const studentScores = students.map((student) => {
      const studentScore = scores.find((score) =>
        score.studentId.equals(student._id)
      );
      return {
        studentId: student._id,
        ...
        scores: studentScore
          ? {
              ...
              fifteenMinutes: studentScore.scores.fifteenMinutes || [
                null,
                null,
                null,
              ],
