...
exports.getexamsubmissionsbystudentid = async (req, res, next) => {
  try {
    ...
    if (!semesterId || !studentId) {
      return res
        .status(400)
        .json({ message: "Semester ID and student ID are required." });
    }
    ...
    if (classes.length === 0) {
      return res.status(404).json({
        message:
          "No classes found for the given student in the specified semester.",
      });
    }
...
exports.submitExamResult = async (req, res) => {
  ...
  try {
    ...
    const classId =
      classIdFromApprovedExam ||
      classIdFromExamSubmission ||
      classIdFromStudent;
    ...
    res.status(200).json({
      message: "Exam result submitted and score updated successfully.",
    });
  } catch (error) {
    console.error("Error submitting exam result:", error);
    res
      .status(500)
      .json({ message: "Error submitting exam result", error: error.message });
  }
};
...
exports.getScores = async (req, res) => {
  try {
    ...
    if (!scores) {
      return res
        .status(404)
        .json({ message: "Scores not found for this student in the class" });
    }
