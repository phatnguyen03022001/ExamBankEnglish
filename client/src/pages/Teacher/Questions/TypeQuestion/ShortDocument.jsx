...
const ShortDocument = ({
  ...
  return (
    <div className="shortDocument">
        ...
        {question.optionsDoc.map((optionDoc, indexOptionDoc) => (
          <div className="flex items-center space-x-2" key={indexOptionDoc}>
            {optionDoc.map((optionDocChil, indexOptionDocChil) => (
              <div
                key={indexOptionDocChil}
                className="flex items-center space-x-2"
              >
                {/* Checkbox for correct answer */}
                <input
                  type="checkbox"
                  id={`ans-${indexOptionDoc}-${indexOptionDocChil}`}
                  name={`correctAnswer-${indexQuestion}`}
                  value={optionDocChil}
                  onChange={(e) =>
                    handleCorrectAnswerChangeDoc(e, indexOptionDoc)
                  }
                  checked={
                    question.answerDoc[indexOptionDoc] === optionDocChil &&
                    optionDocChil !== ""
                  }
                ...
                <input
                  ...
                  value={optionDocChil}
                  onChange={(e) =>
                    handleOptionChange(e, indexOptionDoc, indexOptionDocChil)
                  }
                  className={`w-full text-sm bg-transparent border rounded-lg focus:outline-none focus:ring-0 ${
                    errors[indexOptionDoc] &&
                    errors[indexOptionDoc][indexOptionDocChil]
            ...
            <button
              type="button"
              onClick={() => deleteOptionsDoc(indexOptionDoc)}
      ...
      <div className="mt-3">
        {optionsDocError && (
          <p className="text-red-500">Vui lòng điền đáp án!</p>
        )}
        {answerDoc && (
          <p className="text-red-500">Vui lòng chọn đáp án đúng!</p>
        )}
      </div>
    </div>
  );
