...
function ClassDetails() {
  ...
  const renderChart = () => {
    ...
    const data = {
      ...
      datasets: [
        {
          label: language === 'vi' ? 'Điểm trung bình' : 'Average Score',
    ...
    const options = {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: language === 'vi' ? 'Điểm trung bình của học sinh' : 'Average Scores of Students',
        },
      },
    };
  ...
  if (error) return <p className="text-red-600 dark:text-red-400">{language === 'vi' ? `Lỗi: ${error}` : `Error: ${error}`}</p>;
      ...
      {classDetails ? (
        <div>
          <div className="mb-4">
            ...
            <p>
              {language === 'vi' ? `Trạng thái học kỳ: ${classDetails.semester.isActive ? "Hoạt động" : "Ngừng hoạt động"}` : `Semester Status: ${classDetails.semester.isActive ? "Active" : "Inactive"}`}
            </p>
            <p>
              {language === 'vi' ? `Số học sinh: ${classDetails.students.length}` : `Number of Students: ${classDetails.students.length}`}
            </p>
          </div>
